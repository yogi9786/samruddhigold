...
const Account: React.FC = () => {
  ...
  const handleSaveAddress = async (e: React.FormEvent) => {
    ...
    if (editingAddressIndex !== null) {
      updatedAddresses = addresses.map((addr, i) => i === editingAddressIndex ? newAddress : addr);
    } else {
  ...
  return (
    <div className="min-h-screen bg-[#FFF7F2] flex flex-col font-sans">
      ...
      <div className="flex-grow py-6 sm:py-12 px-3 sm:px-4">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif text-[#5F1517] mb-4 sm:mb-8 text-center">My Account</h1>

          <div className="bg-white rounded-2xl shadow-sm border border-[#5F1517]/10 overflow-hidden flex flex-col md:flex-row min-h-[500px]">

            {/* ── DESKTOP SIDEBAR ── */}
            <div className="hidden md:block w-64 lg:w-72 bg-[#5F1517]/5 p-6 border-r border-[#5F1517]/10 flex-shrink-0">
              <div className="flex items-center gap-4 mb-8">
                <div className="w-14 h-14 bg-[#801416] rounded-full flex items-center justify-center text-white text-xl font-serif shadow-md flex-shrink-0">
                  {user.full_name ? user.full_name.charAt(0).toUpperCase() : user.username.charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0">
                  <h2 className="text-base font-bold text-[#5F1517] truncate">{user.full_name || user.username}</h2>
                  <p className="text-xs text-[#5F1517]/60 truncate break-all">{user.email}</p>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-colors text-sm ${activeTab === 'profile' ? 'bg-[#801416] text-white shadow-sm' : 'text-[#5F1517] hover:bg-white'}`}
                >
                  Profile Details
                </button>
                <button
                  onClick={() => setActiveTab('orders')}
                  className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-colors text-sm ${activeTab === 'orders' ? 'bg-[#801416] text-white shadow-sm' : 'text-[#5F1517] hover:bg-white'}`}
                >
                  My Orders
                </button>
                <button
                  onClick={() => setActiveTab('payments')}
                  className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-colors text-sm ${activeTab === 'payments' ? 'bg-[#801416] text-white shadow-sm' : 'text-[#5F1517] hover:bg-white'}`}
                >
                  Payment History
                </button>
                <button
                  onClick={() => setActiveTab('wishlist')}
                  className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-colors text-sm ${activeTab === 'wishlist' ? 'bg-[#801416] text-white shadow-sm' : 'text-[#5F1517] hover:bg-white'}`}
                >
                  Wishlist
                </button>
                <button
                  onClick={() => setActiveTab('addresses')}
                  className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-colors text-sm ${activeTab === 'addresses' ? 'bg-[#801416] text-white shadow-sm' : 'text-[#5F1517] hover:bg-white'}`}
                >
                  Saved Addresses
                </button>
                <button
                  onClick={() => setActiveTab('security')}
                  className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-colors text-sm ${activeTab === 'security' ? 'bg-[#801416] text-white shadow-sm' : 'text-[#5F1517] hover:bg-white'}`}
                >
                  Security & Password
                </button>
              </div>
              ...
              <div className="pt-6 mt-6 border-t border-[#5F1517]/10">
                <button
                  onClick={handleLogout}
                  className="w-full text-left px-4 py-3 rounded-xl font-semibold text-sm text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors"
                >
                  Sign Out
                </button>
              </div>
            ...
            {/* ── MOBILE DROPDOWN SELECTOR BAR ── */}
            <div className="block md:hidden bg-[#5F1517]/5 p-4 border-b border-[#5F1517]/10">
              <div className="flex items-center justify-between gap-3 mb-3.5">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 bg-[#801416] rounded-full flex items-center justify-center text-white text-base font-serif shadow-sm flex-shrink-0">
                    {user.full_name ? user.full_name.charAt(0).toUpperCase() : user.username.charAt(0).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <h2 className="text-sm font-bold text-[#5F1517] truncate">{user.full_name || user.username}</h2>
                    <p className="text-xs text-[#5F1517]/60 truncate break-all">{user.email}</p>
                  </div>
                </div>
                <button
                  onClick={handleLogout}
                  className="px-3 py-1.5 bg-red-50 border border-red-200 text-red-600 rounded-lg text-xs font-bold hover:bg-red-100 transition flex-shrink-0"
                >
                  Sign Out
                </button>
              </div>
              ...
              <div className="relative">
                <label htmlFor="account-tab-dropdown" className="sr-only">Select Account Section</label>
                <div className="flex items-center bg-white border border-[#801416]/30 rounded-xl px-3.5 py-2.5 shadow-sm">
                  <select
                    id="account-tab-dropdown"
                    value={activeTab}
                    onChange={(e) => setActiveTab(e.target.value)}
                    className="w-full bg-transparent text-[#5F1517] font-bold text-sm focus:outline-none appearance-none cursor-pointer pr-6"
                  >
                  ...
                  <ChevronDown className="w-5 h-5 text-[#801416] absolute right-3 pointer-events-none" />
                </div>
              </div>
            </div>

            <div className="w-full p-4 sm:p-6 md:p-10 flex-grow">

              {activeTab === 'profile' && (
                <div className="animate-in fade-in duration-300">
                  <h3 className="text-xl font-bold text-[#5F1517] mb-2">Profile Information</h3>
                  <p className="text-sm text-[#5F1517]/70 mb-6">
                    Manage your personal account details and WhatsApp phone number for instant order updates & cart notifications.
                  </p>

                  {!profileForm.phone && (
                    <div className="mb-6 p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 flex items-start gap-3 shadow-sm">
                      <span className="text-lg">📱</span>
                      <div>
                        <p className="font-bold text-sm">Action Required: Add your Phone Number</p>
                        <p className="text-xs text-amber-800 mt-0.5">Please add your 10-digit mobile number below to receive instant product reminders & order updates on WhatsApp.</p>
                      </div>
                    </div>
                  )}
                  ...
                  <form onSubmit={handleUpdateProfile} className="space-y-6 mb-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="block text-sm font-semibold text-[#5F1517] mb-1">Full Name <span className="text-red-500">*</span></label>
                        <input
                          type="text"
                          required
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-[#5F1517] mb-1">Email Address <span className="text-red-500">*</span></label>
                        <input
                          type="email"
                          required
                      ...
                      <div className="md:col-span-2">
                        <label className="block text-sm font-semibold text-[#5F1517] mb-1">
                        ...
                        <input
                          ...
                          className={`w-full px-4 py-3 rounded-xl border ${!profileForm.phone ? 'border-amber-400 focus:ring-2 focus:ring-amber-300' : 'border-[#5F1517]/20'} bg-white text-[#5F1517] focus:outline-none focus:border-[#801416] font-medium`}
                        />
                        <p className="text-xs text-gray-500 mt-1">Your 10-digit mobile number for order status and WhatsApp cart notifications.</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-4 flex-wrap pt-2">
                      <button
                        ...
                        className="px-4 sm:px-5 py-2 sm:py-2.5 bg-[#801416] text-white rounded-lg text-xs sm:text-sm font-semibold hover:bg-[#5F1517] transition shadow-sm disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                        {updatingProfile ? (
                          <>
                  ...
                  <div className="pt-6 border-t border-[#5F1517]/10">
                    <button
                      onClick={handleLogout}
              ...
              {activeTab === 'security' && (
                <div className="animate-in fade-in duration-300">
                  <h3 className="text-xl font-bold text-[#5F1517] mb-2">Security & Password</h3>
                  <p className="text-sm text-[#5F1517]/70 mb-6">
                    Manage your account security settings and update your password.
                  </p>

                  <form onSubmit={handleUpdatePassword} className="max-w-md space-y-5 bg-[#5F1517]/5 p-6 rounded-2xl border border-[#5F1517]/10">
                    <div>
                      <label className="block text-sm font-semibold text-[#5F1517] mb-1">New Password</label>
                      <input
                        type="password"
                        required
                        value={newPassword}
                    ...
                    {passwordMessage.text && (
                      <div className={`p-3.5 rounded-xl text-sm font-semibold ${passwordMessage.type === 'error' ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200'}`}>
                        {passwordMessage.text}
                      </div>
                    )}
                    ...
                    <button
                      type="submit"
                      className="px-4 sm:px-5 py-2 sm:py-2.5 bg-[#801416] text-white rounded-lg text-xs sm:text-sm font-semibold hover:bg-[#5F1517] transition shadow-sm"
                    >
                      Update Password
                    </button>
                  </form>
                          ...
                          <div key={idx} className="border border-[#5F1517]/10 rounded-xl p-5 bg-white shadow-sm hover:shadow transition">
                            <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-[#5F1517]/5 pb-3 mb-3 gap-2">
                              ...
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${order.status === 'Payment Successful' || order.status === 'Delivered' ? 'bg-green-100 text-green-800' :
                              ...
                              <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                  <p className="text-xs font-bold text-[#5F1517] flex items-center gap-2">
                                    Shipping Status:
                                    <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${shipStatus === 'Shipped' ? 'bg-purple-100 text-purple-800' :
                ...
                <div className="animate-in fade-in duration-300">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 bg-[#801416] text-white rounded-lg text-xs sm:text-sm font-semibold hover:bg-[#5F1517] transition shadow-sm"
                    >
                      {showAddressForm ? 'Cancel' : <><Plus size={14} className="sm:w-4 sm:h-4" /> Add New Address</>}
                    </button>
                  </div>
                  ...
                  {showAddressForm && (
                    <form onSubmit={handleSaveAddress} className="bg-white p-5 rounded-xl border border-[#5F1517]/20 shadow-sm mb-6 animate-in slide-in-from-top-4 duration-300">
                      <h4 className="font-bold text-[#5F1517] mb-4">
                      ...
                      <div className="mt-4 flex justify-end gap-2.5">
                        <button
                          ...
                          className="px-4 sm:px-5 py-2 sm:py-2.5 bg-gray-100 text-gray-700 text-xs sm:text-sm font-semibold rounded-lg hover:bg-gray-200 transition"
                        >
                          Cancel
                        </button>
                        <button
                          type="submit"
                          className="px-4 sm:px-5 py-2 sm:py-2.5 bg-[#801416] text-white text-xs sm:text-sm font-semibold rounded-lg hover:bg-[#5F1517] transition shadow-sm"
                        >
                  ...
                  {addresses.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {addresses.map((addr, idx) => (
                        <div key={idx} className="bg-white border border-[#5F1517]/15 rounded-xl p-5 relative group shadow-2xs hover:shadow-md transition-all">
                          <div className="absolute top-4 right-4 flex items-center gap-1.5">
                            <button
                              onClick={() => handleEditAddress(idx)}
                              className="p-1.5 text-gray-500 hover:text-[#801416] hover:bg-[#FFF7F2] rounded-lg transition"
                              title="Edit Address"
                            >
                              <Pencil size={15} />
                            </button>
                          ...
                          <span className="inline-block px-2.5 py-0.5 bg-[#5F1517]/5 text-[#5F1517] rounded-md text-xs font-bold uppercase tracking-wider mb-2">{addr.label}</span>
                          <p className="text-[#5F1517] font-semibold text-sm whitespace-pre-line leading-relaxed">{addr.street}</p>
